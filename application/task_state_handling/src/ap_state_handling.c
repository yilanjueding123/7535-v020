...
extern void ap_peripheral_UsbMode_tf_set(INT8U flag);
void ap_state_handling_connect_to_pc(INT32U prev_state)
{
	...
	#else
	OSTimeDly(10); //检测USB还是充电器插入时会init一次 USB（主要是检测PC传来的Reset信号），下面进入USBD的时候还会init一次，两次init间隔不能太近，否则USB硬体会有问题
	//temp = 0;
	//-------------------------------------
	...
	if (nRet >= 0) 
	{
		ap_peripheral_UsbMode_tf_set(1);
		temp = 0;
	}
	else 
	{
		ap_peripheral_uvc_flag_set(1); //无卡时不能切换到USBD模式
		ap_peripheral_UsbMode_tf_set(0);
		temp = 1;
	}
	//-------------------------------------
	#endif
