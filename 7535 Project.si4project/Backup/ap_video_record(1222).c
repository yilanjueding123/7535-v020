...
extern INT8U ap_peripheral_power_off_flag_get(void);
extern INT8U SaveFile_Led_Flag_Get(void);
extern void  SaveFile_Led_Flag_Set(INT8U flag);
extern void led_red_on(void);
...
extern void task_peripheral_lvd_save_cnt_set(INT32U cnt);
		...
		if(ap_state_handling_storage_id_get()!= NO_STORAGE)
            ...
            if(!cyc_restart_flag && ap_state_handling_file_creat_get())
             ...
             if(event != MSG_APQ_VDO_REC_STOP)
             	{
             	   if (ap_peripheral_power_off_flag_get() != 1)
             	   {
				   		if(ap_video_record_sts_get() & VIDEO_RECORD_MD)
						{
...
//					ap_state_handling_icon_show_cmd(ICON_MOTION_DETECT, NULL, NULL);
		...
		task_peripheral_lvd_save_cnt_set(0);  //清除录像时间计数
	}
	video_down_flag=0;
	return STATUS_OK;
}
